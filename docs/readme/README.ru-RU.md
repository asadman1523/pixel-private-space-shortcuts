...
Удерживайте приложение в разблокированном личном пространстве и выберите **Добавить на главный экран**. Используются размещение, база данных, значки и отметка замка Pixel Launcher. Серийный номер профиля и компонент запуска определяют цель и предотвращают дублирование. Поддерживаются перемещение, папки и удаление.
